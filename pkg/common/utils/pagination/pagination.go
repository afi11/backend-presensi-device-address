...
func Paginate(column string, queryString string, value interface{}, pagination *Pagination, db *gorm.DB) func(db *gorm.DB) *gorm.DB {
	var totalRows int64
	...
	return func(db *gorm.DB) *gorm.DB {
		return db.Where(column+" LIKE ?", "%"+queryString+"%").Offset(pagination.GetOffset()).Limit(pagination.GetLimit()).Order(pagination.GetSort())
	}
}
